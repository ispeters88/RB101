...
def pp_14(nested_hsh)
  produce_array = Array.new(0)

  nested_hsh.each_value do |subhash|
    if subhash[:type] == 'fruit'
      produce_array << subhash[:colors].map { |color| color.capitalize }
    elsif subhash[:type] == 'vegetable'
      produce_array << subhash[:size].upcase
    end
  end
