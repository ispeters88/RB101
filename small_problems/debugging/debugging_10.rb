...
# for a particular document in a document collection. 
...
def idf(term, documents)
  ...
  Math.log(number_of_documents / number_of_documents_with_term) rescue 0
end
...
## NEW CODE ##
# > add .to_f to either the nuemrator or denominator in this line:
...
# >> This could also be added in the lines assigning pointers to the variables higher in the same method. Just need a float
# somewhere

## EXPLANATION ##
# the two values in the Math.log call are both integers, and therefore we lose precision on the result. We can fix
# this by turning one into a float, which will result the Math.log method getting a float argument.


## Further exploration ##
# There's still one issue that could get us into trouble when performing division. 
# If you don't see it immediately, remove document1 from the documents collection 
# and check the TF-IDF value of 'quantum' again.

puts tfidf("quantum", document1, documents[1..]) # ~ 5.5
puts tfidf("quantum", document2, documents[1..]) # 0
puts tfidf("quantum", document3, documents[1..]) # 0


# > It is possible that the search term appears in none of the documents. In this case, the current code will result
# in a ZeroDivisionError. 
# This has been updated using rescue 0, to return 0 if the denoninator value is 0